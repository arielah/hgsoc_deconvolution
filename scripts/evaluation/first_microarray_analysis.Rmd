---
title: "First TCGA microarray analysis"
author: "Ariel Hippen"
date: '2023-01-28'
output:  
  pdf_document:
...
Having run the TCGA microarray data through BayesPrism, this notebook compares the samples' cell type composition with their subtype annotations from the Way pipeline and the patients' survival status/time.
...
plotfile <- paste(plot_path, "evaluation_plots",
                  "microarray_proportion_barchart.png", sep = "/")
...
```{r}
# Load survival data
tcga_survival <- fread(paste(local_data_path, "TCGA", "microarray_survival.tsv", 
                             sep = "/"))
tcga_survival$unique_patient_ID <- gsub("-","\\.", tcga_survival$unique_patient_ID)
```

```{r}
# Combine survival data with %
tcga_t$unique_patient_ID <- rownames(tcga_t)
...
```{r}
g <- ggplot(tcga_master, mapping = aes(x=tcga_master$days_to_death, y=tcga_master$Fibroblasts)) +
  geom_point() + xlab("OS (days)") + ylab("% Fibroblasts")
plotfile <- paste(plot_path, "evaluation_plots", "microarray_survival_by_fibroblasts.png", sep = "/")
png(filename = plotfile); g; dev.off()
g

g <- ggplot(tcga_master, mapping = aes(x=tcga_master$days_to_death, y=tcga_master$`Epithelial cells`)) +
  geom_point() + xlab("OS (days)") + ylab("% Epithelial cells")
plotfile <- paste(plot_path, "evaluation_plots", "microarray_survival_by_epithelial.png", sep = "/")
png(filename = plotfile); g; dev.off()
g

g <- ggplot(tcga_master, mapping = aes(x=tcga_master$days_to_death, y=tcga_master$Immune)) +
  geom_point() + xlab("OS (days)") + ylab("% Immune")
plotfile <- paste(plot_path, "evaluation_plots", "microarray_survival_by_immune.png", sep = "/")
png(filename = plotfile); g; dev.off()
g

g <- ggplot(tcga_master, mapping = aes(x=tcga_master$days_to_death, y=tcga_master$`Endothelial cells`)) +
  geom_point() + xlab("OS (days)") + ylab("% Endothelial cells")
plotfile <- paste(plot_path, "evaluation_plots", "microarray_survival_by_endothelial_cells.png", sep = "/")
png(filename = plotfile); g; dev.off()
g
```
...
tcga_master$vital_status <- recode(tcga_master$vital_status,
                                   "deceased"=1,
                                   "living"=0)

km <- Surv(time = tcga_master$days_to_death,  event = tcga_master$vital_status)
km_treatment<-survfit(km~high_fibro,data=tcga_master,type='kaplan-meier',conf.type='log')

plotfile <- paste(plot_path, "evaluation_plots", "microarray_KaplanMeier_fibroblasts.png", sep = "/")
...
plotfile <- paste(plot_path, "evaluation_plots", "microarray_fibroblasts_by_subtype.png", sep = "/")
png(filename = plotfile); g; dev.off()
g

g <- ggplot(tcga_t, mapping = aes(x=Subtype, y=`Epithelial cells`)) + geom_boxplot()
plotfile <- paste(plot_path, "evaluation_plots", "microarray_epithelial_by_subtype.png", sep = "/")
png(filename = plotfile); g; dev.off()
g

g <- ggplot(tcga_t, mapping = aes(x=Subtype, y=Immune)) + geom_boxplot()
plotfile <- paste(plot_path, "evaluation_plots", "microarray_immune_by_subtype.png", sep = "/")
png(filename = plotfile); g; dev.off()
g

g <- ggplot(tcga_t, mapping = aes(x=Subtype, y=Macrophages)) + geom_boxplot()
plotfile <- paste(plot_path, "evaluation_plots", "microarray_macrophages_by_subtype.png", sep = "/")
png(filename = plotfile); g; dev.off()
g

g <- ggplot(tcga_t, mapping = aes(x=Subtype, y=`T cells`)) + geom_boxplot()
plotfile <- paste(plot_path, "evaluation_plots", "microarray_tcells_by_subtype.png", sep = "/")
png(filename = plotfile); g; dev.off()
g

g <- ggplot(tcga_t, mapping = aes(x=Subtype, y=`Endothelial cells`)) + geom_boxplot()
plotfile <- paste(plot_path, "evaluation_plots", "microarray_endothelial_by_subtype.png", sep = "/")
png(filename = plotfile); g; dev.off()
g
```
...
plotfile <- paste(plot_path, "evaluation_plots", "microarray_fibroblasts_by_subtype_k3.png", sep = "/")
png(filename = plotfile); g; dev.off()
g

g <- ggplot(tcga_t, mapping = aes(x=factor(ClusterK3_kmeans), y=`Epithelial cells`)) + geom_boxplot()
plotfile <- paste(plot_path, "evaluation_plots", "microarray_epithelial_by_subtype_k3.png", sep = "/")
png(filename = plotfile); g; dev.off()
g

g <- ggplot(tcga_t, mapping = aes(x=factor(ClusterK3_kmeans), y=Immune)) + geom_boxplot()
plotfile <- paste(plot_path, "evaluation_plots", "microarray_immune_by_subtype_k3.png", sep = "/")
png(filename = plotfile); g; dev.off()
g

g <- ggplot(tcga_t, mapping = aes(x=factor(ClusterK3_kmeans), y=Macrophages)) + geom_boxplot()
plotfile <- paste(plot_path, "evaluation_plots", "microarray_macrophages_by_subtype_k3.png", sep = "/")
png(filename = plotfile); g; dev.off()
g

g <- ggplot(tcga_t, mapping = aes(x=factor(ClusterK3_kmeans), y=`T cells`)) + geom_boxplot()
plotfile <- paste(plot_path, "evaluation_plots", "microarray_tcells_by_subtype_k3.png", sep = "/")
png(filename = plotfile); g; dev.off()
g

g <- ggplot(tcga_t, mapping = aes(x=factor(ClusterK3_kmeans), y=`Endothelial cells`)) + geom_boxplot()
plotfile <- paste(plot_path, "evaluation_plots", "microarray_endothelial_by_subtype_k3.png", sep = "/")
png(filename = plotfile); g; dev.off()
g
```
