...
sce <- readRDS(paste(local_data_path,"sce_objects/2467_clustered.rds",sep="/"))

plotUMAP(sce, colour_by="clusters")

ct <- fread(paste(local_data_path,"celltypist_output/2467_predicted_labels.csv", sep="/"))
...
ct[ct$clusters %in% c(1, 3, 4, 8) & ct$Simplified %in% c("Fibroblasts"),]$keep <- TRUE
ct[ct$clusters %in% c(2, 7) & ct$Simplified %in% c("Endothelial cells"),]$keep <- TRUE
ct[ct$clusters %in% c(7) & ct$Simplified %in% c("Epithelial cells"),]$keep <- TRUE
ct[ct$clusters %in% c(5) & ct$Simplified %in% c("Monocytes", "Macrophages",
                                                "DC"),]$keep <- TRUE
ct[ct$clusters %in% c(6, 9) & ct$Simplified %in% c("T cells", "NK cells",
                                                   "Mast cells", "B cells"),]$keep <- TRUE
ct[ct$clusters %in% c(10) & ct$Simplified %in% c("Plasma cells"), ]$keep <- TRUE
...
saveRDS(sce, paste(local_data_path, "sce_objects/2467_labeled.rds", sep = "/"))
