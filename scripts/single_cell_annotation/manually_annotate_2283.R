...
sce <- readRDS(paste(local_data_path,"sce_objects/2283_clustered.rds",sep="/"))

plotUMAP(sce, colour_by="clusters")

ct <- fread(paste(local_data_path,"celltypist_output/2283_predicted_labels.csv", sep="/"))
...
ct[ct$clusters %in% c(1, 7) & ct$Simplified %in% c("Epithelial cells"), ]$keep <- TRUE
ct[ct$clusters %in% c(4) & ct$Simplified %in% c("Macrophages", "Monocytes"), ]$keep <- TRUE
ct[ct$clusters %in% c(3, 5) & ct$Simplified %in% c("Endothelial cells"), ]$keep <- TRUE
ct[ct$clusters %in% c(2, 3, 5, 6) & ct$Simplified %in% c("Fibroblasts"), ]$keep <- TRUE
ct[ct$clusters %in% c(8) & ct$Simplified %in% c("T cells", "NK cells"), ]$keep <- TRUE
...
saveRDS(sce, paste(local_data_path, "sce_objects/2283_labeled.rds", sep = "/"))
