...
sce <- readRDS(paste(local_data_path,"sce_objects/19833X1_clustered.rds",sep="/"))

plotUMAP(sce, colour_by="clusters")

ct <- fread(paste(local_data_path,"celltypist_output/19833X1_predicted_labels.csv", sep="/"))
...
ct[ct$clusters %in% c(2, 3) & ct$Simplified %in% c("Monocytes", "Macrophages",
                                                   "DC", "pDC"),]$keep <- TRUE
ct[ct$clusters %in% c(4, 5) & ct$Simplified %in% c("T cells", "B cells",
                                                   "NK cells", "pDC"),]$keep <- TRUE
...
saveRDS(sce, paste(local_data_path, "sce_objects/19833X1_labeled.rds", sep = "/"))
