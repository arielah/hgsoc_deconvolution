...
sce <- readRDS(paste(local_data_path,"sce_objects/18389X2_clustered.rds",sep="/"))

plotUMAP(sce, colour_by="clusters")

ct <- fread(paste(local_data_path,"celltypist_output/18389X2_predicted_labels.csv", sep="/"))
...
ct[ct$clusters %in% c(1, 4) & ct$Simplified %in% c("T cells", "Macrophages"),]$keep <- TRUE
ct[ct$clusters %in% c(3,4) & ct$Simplified %in% c("Fibroblasts", "Endothelial cells"),]$keep <- TRUE
...
saveRDS(sce, paste(local_data_path, "sce_objects/18389X2_labeled.rds", sep = "/"))
