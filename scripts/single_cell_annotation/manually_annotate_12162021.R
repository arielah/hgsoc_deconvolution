...
sce <- readRDS(paste(local_data_path,"sce_objects/12162021_clustered.rds",sep="/"))
sce$Sample <- "12162021"

plotUMAP(sce, colour_by="clusters")

ct <- fread(paste(local_data_path,"celltypist_output/12162021_predicted_labels.csv", sep="/"))
...
# Switch NK cells to smooth muscle
ct[ct$Simplified=="NK cells" & ct$clusters==2,]$Simplified <- "Smooth muscle cells"
sce[, sce$CT=="NK cells" & sce$clusters==2]$CT <- "Smooth muscle cells"



ct[ct$clusters %in% c(1, 3, 4) & ct$Simplified %in% c("B cells", "NK cells", "T cells",
                                                      "Mast cells"),]$keep <- TRUE
ct[ct$clusters %in% c(2, 7) & ct$Simplified %in% c("Smooth muscle cells",
                                                   "Fibroblasts"),]$keep <- TRUE
ct[ct$clusters %in% c(4) & ct$Simplified %in% c("Plasma cells"),]$keep <- TRUE
ct[ct$clusters %in% c(5) & ct$Simplified %in% c("Endothelial cells"),]$keep <- TRUE
ct[ct$clusters %in% c(5, 9, 10) & ct$Simplified %in% c("Epithelial cells"),]$keep <- TRUE
ct[ct$clusters %in% c(6) & ct$Simplified %in% c("Monocytes", "Macrophages",
                                                       "DC"),]$keep <- TRUE
ct[ct$clusters %in% c(8) & ct$Simplified %in% c("pDC"),]$keep <- TRUE
...
labels <- fread(paste(data_path,"pooled_tumors/12162021/vireo/chunk_ribo/donor_ids.tsv", sep = "/"))
labels$Sample <- "12162021"
...
saveRDS(sce, paste(local_data_path, "sce_objects/12162021_labeled.rds", sep = "/"))
