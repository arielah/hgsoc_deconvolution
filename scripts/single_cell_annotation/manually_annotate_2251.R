...
sce <- readRDS(paste(local_data_path,"sce_objects/2251_clustered.rds",sep="/"))

plotUMAP(sce, colour_by="clusters")

ct <- fread(paste(local_data_path,"celltypist_output/2251_predicted_labels.csv", sep="/"))
...
ct[ct$clusters %in% c(1) & ct$Simplified %in% c("Plasma cells"),]$keep <- TRUE
ct[ct$clusters %in% c(2) & ct$Simplified %in% c("B cells", "pDC"),]$keep <- TRUE
ct[ct$clusters %in% c(5) & ct$Simplified %in% c("T cells", "NK cells", "ILC",
                                                "Mast cells"),]$keep <- TRUE
ct[ct$clusters %in% c(7) & ct$Simplified %in% c("Endothelial cells"),]$keep <- TRUE
ct[ct$clusters %in% c(8) & ct$Simplified %in% c("Macrophages", "Monocytes",
                                                "DC"),]$keep <- TRUE
ct[ct$clusters %in% c(9) & ct$Simplified %in% c("Fibroblasts"),]$keep <- TRUE
ct[ct$clusters %in% c(11) & ct$Simplified %in% c("Epithelial cells"),]$keep <- TRUE
...
saveRDS(sce, paste(local_data_path, "sce_objects/2251_labeled.rds", sep = "/"))
