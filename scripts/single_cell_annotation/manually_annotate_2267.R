...
sce <- readRDS(paste(local_data_path,"sce_objects/2267_clustered.rds",sep="/"))

plotUMAP(sce, colour_by="clusters")

ct <- fread(paste(local_data_path,"celltypist_output/2267_predicted_labels.csv", sep="/"))
...
ct[ct$clusters %in% c(1) & ct$Simplified %in% c("T cells", "NK cells"), ]$keep <- TRUE
ct[ct$clusters %in% c(2, 7) & ct$Simplified %in% c("Monocytes", "Macrophages",
                                                   "DC"), ]$keep <- TRUE
ct[ct$clusters %in% c(3, 7) & ct$Simplified %in% c("Epithelial cells"),]$keep <- TRUE
ct[ct$clusters %in% c(4, 5) & ct$Simplified %in% c("T cells", "Plasma cells",
                                                   "pDC"), ]$keep <- TRUE
ct[ct$clusters %in% c(6) & ct$Simplified %in% c("B cells"), ]$keep <- TRUE
ct[ct$clusters %in% c(7) & ct$Simplified %in% c("Fibroblasts", "Endothelial cells"), ]$keep <- TRUE
...
saveRDS(sce, paste(local_data_path, "sce_objects/2267_labeled.rds", sep = "/"))
