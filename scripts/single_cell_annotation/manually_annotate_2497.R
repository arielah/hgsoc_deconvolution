...
sce <- readRDS(paste(local_data_path,"sce_objects/2497_clustered.rds",sep="/"))

plotUMAP(sce, colour_by="clusters")

ct <- fread(paste(local_data_path,"celltypist_output/2497_predicted_labels.csv", sep="/"))
...
# Switch cluster 6 to smooth muscle
ct[ct$clusters==6,]$Simplified <- "Smooth muscle cells"
sce[, sce$clusters==6,]$CT <- "Smooth muscle cells"


ct[ct$clusters %in% c(2, 8) & ct$Simplified %in% c("T cells", "ILC", "Mast cells",
                                                   "NK cells"),]$keep <- TRUE
ct[ct$clusters %in% c(3, 4) & ct$Simplified %in% c("Epithelial cells"),]$keep <- TRUE
...
ct[ct$clusters %in% c(7, 9) & ct$Simplified %in% c("Fibroblasts"), ]$keep <- TRUE
ct[ct$clusters %in% c(10) & ct$Simplified %in% c("B cells"),]$keep <- TRUE
ct[ct$clusters %in% c(11) & ct$Simplified %in% c("Macrophages", "Monocytes", 
                                                 "DC"),]$keep <- TRUE
...
saveRDS(sce, paste(local_data_path, "sce_objects/2497_labeled.rds", sep = "/"))
