...
sce <- readRDS(paste(local_data_path,"sce_objects/16030X3_clustered.rds",sep="/"))

plotUMAP(sce, colour_by="clusters")

ct <- fread(paste(local_data_path,"celltypist_output/16030X3_predicted_labels.csv", sep="/"))
...
ct[ct$clusters %in% c(1, 3) & ct$Simplified %in% c("Macrophages", "DC"),]$keep <- TRUE
ct[ct$clusters %in% c(2, 3) & ct$Simplified %in% c("Epithelial cells", "Endothelial cells"),]$keep <- TRUE
ct[ct$clusters %in% c(3, 4) & ct$Simplified %in% c("T cells", "NK cells"),]$keep <- TRUE
...
saveRDS(sce, paste(local_data_path, "sce_objects/16030X3_labeled.rds", sep = "/"))